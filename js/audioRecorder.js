...
function recordAudio() {
    ...
    mediaRecorder.ondataavailable = function (blob) {
        ...
        xhr('/preslikave/server/save.php', formData, function (fileURL) {
        });
...
function makePOSTRequest(formData) {
    ...
    xhrReq.open('POST', '/preslikave/server/upload_images.php');
...
function makePOSTRequestDB(formData) {
    var xhrReq = new XMLHttpRequest();
    xhrReq.open('POST', '/preslikave/server/database_insert.php');

    xhrReq.onreadystatechange = function () {
        if (xhrReq.readyState == 4 && xhrReq.status == 200) {
            console.log("200 - zapis v bazo");
        }
    };
...
function saveImages() {
    ...
    var fileTypeI = 'image';
    var fileNameI = fileNameOfSaving + '_user' + '.png';
    ...
    formDataI.append('leva_barva', colorLeftGlobalHEX);
    formDataI.append('desna_barva', colorRightGlobalHEX);
    formDataI.append('povprecna_barva', colorAverageGlobalHEX);
